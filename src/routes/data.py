    ...
    try:
        async with aiofiles.open(file_path,"wb") as f:
         ...
         return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={
                "signal": ResponseSignal.FILE_UPOLAD_FAILED.value
            }
        )
...
#store the assets into the database
    ...
    asset_record =await asset_model.create_asset(asset=asser_resource)


    return JSONResponse(
            content={
                "signal": ResponseSignal.FILE_UPOLAD_SUCCESS.value,
                "file_id": str( asset_record.asset_id),
            }
        )
    
@data_router.post("/process/{project_id}")
async def process_endpoint(project_id: int, process_request: ProccessRequest,
                           request: Request):
